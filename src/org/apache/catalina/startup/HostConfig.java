...
public class HostConfig implements LifecycleListener, Runnable {
	...
	/**
	 * Should we monitor the <code>appBase</code> directory for new applications
	...
	private void deployDescriptors(File appBase, String[] files) {
		if (!deployXML)
			return;
	...
	/**
	 * Expand the WAR file found at the specified URL into an unpacked directory
	 * structure, and return the absolute pathname to the expanded directory.
	 *
	 * @param war
	 *            URL of the web application archive to be expanded (must start
	 *            with "jar:")
	 *
	...
	protected String expand(URL war) throws IOException {
		...
		/*
		 * JarFile jarFile = juc.getJarFile(); if (getDebug() >= 2) { log(
		 * "  Have opened JAR file successfully"); } Enumeration jarEntries =
		 * jarFile.entries(); if (getDebug() >= 2) { log(
		 * "  Have retrieved entries enumeration"); } while
		 * (jarEntries.hasMoreElements()) { JarEntry jarEntry = (JarEntry)
		 * jarEntries.nextElement(); String name = jarEntry.getName(); if
		 * (getDebug() >= 2) { log("  Am processing entry " + name); } int last
		 * = name.lastIndexOf('/'); if (last >= 0) { File parent = new
		 * File(docBase, name.substring(0, last)); if (getDebug() >= 2) { log(
		 * "  Creating parent directory " + parent); } parent.mkdirs(); } if
		 * (name.endsWith("/")) { continue; } if (getDebug() >= 2) { log(
		 * "  Creating expanded file " + name); } InputStream input =
		 * jarFile.getInputStream(jarEntry); expand(input, docBase, name);
		 * input.close(); } jarFile.close();
		 */
	...
	protected void undeployApps() {
		...
		for (int i = 0; i < contextPaths.length; i++) {
			...
			try {
				...
				log(sm.getString("hostConfig.undeploy.error", contextPaths[i]), t);
			}
		}
	...
	/**
	 * Start the background thread that will periodically check for web
	 * application autoDeploy and changes to the web.xml config.
	 *
	...
	/**
	 * Stop the background thread that is periodically checking for for web
	 * application autoDeploy and changes to the web.xml config.
	 */
