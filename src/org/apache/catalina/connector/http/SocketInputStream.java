   ...
   protected byte[] buf;
	...
	public void readHeader(HttpHeader header) throws IOException {
		...
		while(!colon){
			...
			//We're the end of the internal buffer
			if(pos >= count){
				int val = read();
				if(val == -1){
					throw new IOException("requestStream.readline.error");
...
//		System.out.println(new String(header.name));
		...
		while(validLine){
			boolean space = true;
			//Skip spaces
			//Note: Only leading white spacecs are removed. Trailing white
			//spaces are not
			while(space){
				//We're at the end of the internal buffer
				if(pos >= count){
					//copy part (or all) of the internal buffer to the line buffer
					int val = read();
					if(val == -1){
			...
			while(!eol){
				...
				if(buf[pos] == CR){
					...
					//FIXME: check if binary coversion is working fine
					int ch = buf[pos] & 0xff;
